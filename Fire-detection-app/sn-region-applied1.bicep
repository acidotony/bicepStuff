...
metadata description = 'This template defines the Selection Navigator regional resources for Applied applications.'

// Functions
@description('Picker function to filter values by selector.')
...
var environment = loadJsonContent('../../../fire-detection/applied_app/environment.json')['sn-${environmentName}']
var location = az.resourceGroup().location
////////var logAnalyticsWorkspaces = loadJsonContent('../data/logAnalyticsWorkspace.json')
var logAnalyticsWorkspaces = loadJsonContent('../../../fire-detection/applied_app/logAnalyticsWorkspace.json')
...
var sharedStorageAccountNameValue = sharedStorageAccountName ?? 'stsn${environment.name}${location}03062025'
var appliedGroupRoleAssignments = [{ group: 'SelNav-Azure-AppDev-AppliedDX', roles: ['Contributor'], filters: ['dev2','dev','qa']  }
]

var groupName = 'AppliedDX'
...
module sqlDatabasesModule '../modules/sn-sqlDatabase.bicep' = [for sqlDatabase in filteredSqlDatabases: {
  ...
  params: {
    ...
    sqlServerName: 'sn-${environment.name}-sql-${format('{0:000}03062025', sn.ResourceGroupIndex(az.resourceGroup().name))}'
