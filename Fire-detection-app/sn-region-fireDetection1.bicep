...
// Variables
var environment = loadJsonContent('../../../fire-detection/environment.json')['sn-${environmentName}']
var location = az.resourceGroup().location
var logAnalyticsWorkspaces = loadJsonContent('../../../fire-detection/logAnalyticsWorkspace.json')
...
module sqlDatabasesModule '../modules/sn-sqlDatabase.bicep' = [for sqlDatabase in filteredSqlDatabases: {
  ...
  params: {
    ...
    sqlServerName: 'sn-${environment.name}-sql-${format('{0:000}', sn.ResourceGroupIndex(az.resourceGroup().name))}03062025'
...
module applicationsModule './sn-site.bicep' = [
  for application in filteredApplications: {
    ...
    params: {
      name: '${sn.CreateSiteName(environment.name, location, application.type, application.abbr)}-03062025'
