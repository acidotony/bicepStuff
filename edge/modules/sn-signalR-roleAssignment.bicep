...
// Metadata
metadata description = 'This template defines role based access control (RBAC) rights for signalR resource.'
...
module appSettingsModule './sn-site-config-appsettings.bicep' = [for siteSetting in siteSettingsFlattened: {
  ...
  params: {
    ...
    appSettings: toObject([{name: siteSetting.appSettingName, value:  'https://${signalRService.name}.service.signalr.net'}], arg => arg.name, arg => arg.value)
