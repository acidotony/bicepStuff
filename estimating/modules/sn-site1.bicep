...
var relay = loadJsonContent('../../../../bicepStuff/relay.json')[environment.relay]
...
var appInsightsName =  toUpper('sn-${environment.name}-${appAbbreviation}')
...
var linkedHybridConnection = [
  for hybridConnection in filteredHybridConnections: length(filter(relay.hybridConnections, hc => hc.hostname == hybridConnection.hostname && hc.port == hybridConnection.port)) > 0 ?    {
...
module applicationInsights 'sn-applicationInsights.bicep' = {  
...
resource hybridConnectionResources 'Microsoft.Web/sites/hybridConnectionNamespaces/relays@2024-04-01' = [for hybridConnection in linkedHybridConnection: if (hybridConnection.name != '') {
...
var applicationInsightsSettings = {
  APPLICATIONINSIGHTS_CONNECTION_STRING: applicationInsights.outputs.connectionString // preferred value
  APPINSIGHTS_CONNECTIONSTRING: applicationInsights.outputs.connectionString // Legacy value
  APPINSIGHTS_INSTRUMENTATIONKEY: applicationInsights.outputs.instrumentationKey // Legacy Legacy value
...
module originGroupModule './sn-frontDoor-originGroup.bicep' = {
  ...
  params: {
    frontDoorName: environment.frontDoor.name
...
//     patternsToMatch: existingPatternsToMatch
//     customDomainNames: [
