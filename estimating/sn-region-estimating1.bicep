...
var sharedStorageAccountNameValue = sharedStorageAccountName ?? 'stsn${environment.name}${location}aj0'
var EstimatingGroupRoleAssignments = [{ group: 'SelNav-Azure-AppDev-Estimating', roles: ['Contributor'], filters: ['dev2','dev','qa']  }
]

// Filter the group roles by environment
var groupName = 'Estimating'
...
module applicationsModule './sn-site1.bicep' = [
  for application in filteredApplications: {
    ...
    params: {
      ...
      groupRoleAssignments: union(EstimatingGroupRoleAssignments, application.?groupRoleAssignments ?? [])
...
module EstimatingEventHubs 'sn-eventhub.bicep' = [for eventhub in eventHubs: {
  name: 'eh-sn-${environment.name}-${eventhub.name}-${location}'
  params: {
    environmentName: environment.name
    tags: tags
    ///// name: 'sn-${environment.name}-${eventhub.name}-${location}'
    name: 'eh-sn-${environment.name}-${eventhub.name}-${location}'
    eventHubName: eventhub.name
    sku: 'Standard'
    capacity: 1
    groupRoleAssignments : EstimatingGroupRoleAssignments
